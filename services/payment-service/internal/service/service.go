...
func (s *paymentService) ConfirmCash(ctx context.Context, paymentID, driverID string) (*model.Payment, error) {
	...
	if p.Status != model.StatusAwaitingCashConfirm && p.Status != model.StatusPending && p.Status != model.StatusProcessing {
		return nil, fmt.Errorf("payment is in status %s; expected PENDING, PROCESSING, or AWAITING_CASH_CONFIRM", p.Status)
	}
	now := time.Now()
