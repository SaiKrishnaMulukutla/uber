...
func (h *Handler) GetByTripID(w http.ResponseWriter, r *http.Request) {
	...
	if claims.UserID != p.RiderID && claims.UserID != p.DriverID {
		httputil.WriteJSON(w, http.StatusForbidden, map[string]string{"error": "not your payment"})
		return
	}
...
func (h *Handler) CreateOrder(w http.ResponseWriter, r *http.Request) {
	...
	if claims.Role != "rider" {
		httputil.WriteJSON(w, http.StatusForbidden, map[string]string{"error": "only riders can create orders"})
		return
	}
	var body struct {
	...
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil || body.PaymentID == "" {
		httputil.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "payment_id is required"})
		return
	}
...
func (h *Handler) VerifyPayment(w http.ResponseWriter, r *http.Request) {
	...
	if req.PaymentID == "" || req.ProviderOrderID == "" || req.ProviderPaymentID == "" || req.Signature == "" {
		httputil.WriteJSON(w, http.StatusBadRequest, map[string]string{"error": "payment_id, provider_order_id, provider_payment_id, and signature are required"})
		return
	}
...
func (h *Handler) Simulate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httputil.WriteJSON(w, http.StatusNotFound, map[string]string{"error": "payment not found"})
		return
	}
...
func (h *Handler) ConfirmCash(w http.ResponseWriter, r *http.Request) {
	...
	if claims.Role != "driver" {
		httputil.WriteJSON(w, http.StatusForbidden, map[string]string{"error": "only drivers can confirm cash payments"})
		return
	}
	...
	httputil.WriteJSON(w, http.StatusOK, map[string]any{"status": p.Status, "amount": p.Amount})
}
